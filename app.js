...
const LAST_DATA_UPDATE_AT = '2026-09-05T07:07:02+08:00';
const CURRENT_LAYOUT_VERSION = '2026-09-v2';
const HISTORY_STORAGE_KEY = 'market-pulse-history-v2';
const HISTORY_TRASH_STORAGE_KEY = 'market-pulse-history-trash-v1';
...
function loadHistorySnapshots() { let saved = {}; let trash = []; try { saved = JSON.parse(storage.getItem(HISTORY_STORAGE_KEY) || '{}'); } catch (_) {} try { trash = JSON.parse(storage.getItem(HISTORY_TRASH_STORAGE_KEY) || '[]'); } catch (_) {} state.history = saved && typeof saved === 'object' && !Array.isArray(saved) ? saved : {}; state.historyTrash = Array.isArray(trash) ? trash : []; }
function saveHistorySnapshots() { try { storage.setItem(HISTORY_STORAGE_KEY, JSON.stringify(state.history)); storage.setItem(HISTORY_TRASH_STORAGE_KEY, JSON.stringify(state.historyTrash)); } catch (_) {} }
...
function historyLabel(date) { const records = historyVersions(date); const record = resolveSnapshot(date, 'latest'); return record ? `${formatMonth(parseDate(date))} · ${records.length} 条记录 · ${record.events.length} 项事件` : formatMonth(parseDate(date)); }
function historySnapshotCount() { return historyDates().length; }
function historyRecordCount() { return historyDates().reduce((total, date) => total + historyVersions(date).length, 0); }
...
function updateToolbarMonth() { const monthPrefix = `${state.cursor.getFullYear()}-${String(state.cursor.getMonth() + 1).padStart(2, '0')}`; document.getElementById('monthTitle').textContent = formatMonth(state.cursor); const mode = state.historyDate ? `历史快照 · ${state.historyDate} · 序号 ${((state.historyVersion ?? 0) + 1)}` : '实时数据'; document.getElementById('monthMeta').innerHTML = `<span class="month-meta-item">${visibleEvents().filter(event => eventDisplayDate(event).startsWith(monthPrefix)).length} 项事件</span><span class="month-meta-item">${escapeHtml(timezoneLabel(state.timezone))}</span><span class="month-meta-item">${escapeHtml(mode)}</span>`; }
...
function renderChangelogControls(current, previous) { const controls = document.getElementById('changelogControls'); const beforeNode = document.getElementById('changelogBeforeTime'); const afterSelect = document.getElementById('changelogAfterVersion'); if (!controls || !beforeNode || !afterSelect) return; if (!current) { controls.hidden = true; return; } const records = historyVersions(current.date); if (!records.length) { controls.hidden = true; return; } const selectedIndex = Math.max(0, Math.min(current.versionIndex ?? records.length - 1, records.length - 1)); const previousRecord = previous; beforeNode.textContent = previousRecord ? formatSnapshotTime(previousRecord) : '没有更早记录'; afterSelect.innerHTML = records.map((record, index) => `<option value="${index}">序号 ${index + 1} · ${formatSnapshotTime(record)}</option>`).join(''); afterSelect.value = String(selectedIndex); controls.hidden = false; }
function renderChangelog() { const node = document.getElementById('changelogList'); const meta = document.getElementById('changelogMeta'); if (!node || !meta) return; const current = activeSnapshot(); if (!current || !state.historyDate && !state.history[todayKey()]) { meta.textContent = '尚未保存今日快照'; renderChangelogControls(null, null); node.innerHTML = '<div class="empty-state">数据更新或导入新数据后会保存记录，之后可在这里查看差异；刷新和回溯不会新增记录。</div>'; return; } const currentDate = current.date || calendarTodayKey(); const previous = previousSnapshotFor(current); const previousDate = previous?.date || null; const diff = activeDiff(); meta.textContent = `${state.historyDate ? '历史快照' : '今日快照'} · ${currentDate} · 序号 ${((current.versionIndex ?? 0) + 1)} · 布局 ${current.layoutVersion || CURRENT_LAYOUT_VERSION}`; renderChangelogControls(current, previous); if (!diff.hasChanges) { node.innerHTML = `<div class="changelog-empty">相对上一份已保存快照，没有检测到核心信息变化。<span>（已保存快照 ${historySnapshotCount()} 天 · ${historyRecordCount()} 条记录）</span></div>`; return; } const sections = []; if (previous && previous.layoutVersion !== current.layoutVersion) sections.push(`<div class="changelog-section"><h4>页面结构 · 1 项</h4><article class="changelog-item changed"><div class="changelog-grid"><div class="changelog-grid-head">序号</div><div class="changelog-grid-head">更新前</div><div class="changelog-grid-head">更新后</div><div class="changelog-grid-head">变更原因</div><div class="changelog-grid-cell changelog-sequence">1</div><div class="changelog-grid-cell"><strong>页面布局</strong><del>${escapeHtml(previous.layoutVersion || '—')}</del></div><div class="changelog-grid-cell"><strong>页面布局</strong><strong>${escapeHtml(current.layoutVersion || '—')}</strong></div><div class="changelog-grid-cell changelog-reason">页面结构或显示规则发生更新。</div></div></article></div>`); if (diff.added.length) sections.push(`<div class="changelog-section"><h4>新增事件 · ${diff.added.length} 项</h4>${diff.added.map((event, index) => renderChangeLogItem(event, 'added', [], {}, {}, index + 1)).join('')}</div>`); if (diff.removed.length) sections.push(`<div class="changelog-section"><h4>移除事件 · ${diff.removed.length} 项</h4>${diff.removed.map((event, index) => renderChangeLogItem(event, 'removed', [], {}, {}, index + 1)).join('')}</div>`); if (diff.changed.length) sections.push(`<div class="changelog-section"><h4>信息变更 · ${diff.changed.length} 项</h4>${diff.changed.map((item, index) => renderChangeLogItem(item.after, 'changed', item.fields, item.before, item.after, index + 1)).join('')}</div>`); node.innerHTML = `<div class="changelog-compare">更新前：${previous ? formatSnapshotTime(previous) : '没有更早记录'} · 更新后：${formatSnapshotTime(current)}<br />对比基准：${previousDate || '没有更早的快照'}</div>${sections.join('')}`; }
function renderHistoryPicker() { const title = document.getElementById('historyMonthTitle'); const grid = document.getElementById('historyCalendarGrid'); const note = document.getElementById('historyNote'); if (!title || !grid || !note) return; const cursor = new Date(state.historyCursor.getFullYear(), state.historyCursor.getMonth(), 1); title.textContent = formatMonth(cursor); grid.innerHTML = ''; const offset = (cursor.getDay() + 6) % 7; for (let index = 0; index < offset; index += 1) { const placeholder = document.createElement('span'); placeholder.className = 'history-day placeholder'; grid.appendChild(placeholder); } const last = new Date(cursor.getFullYear(), cursor.getMonth() + 1, 0).getDate(); const saved = new Set(historyDates()); for (let day = 1; day <= last; day += 1) { const date = keyFromParts(cursor.getFullYear(), cursor.getMonth() + 1, day); const button = document.createElement('button'); button.type = 'button'; button.className = `history-day${saved.has(date) ? ' has-snapshot' : ''}${date === state.historyDate ? ' selected' : ''}`; button.textContent = String(day); button.disabled = !saved.has(date); button.title = saved.has(date) ? historyLabel(date) : '这一天尚未保存快照'; if (saved.has(date)) button.addEventListener('click', () => enterHistory(date)); grid.appendChild(button); } note.textContent = `已保存 ${historySnapshotCount()} 天、${historyRecordCount()} 条记录；只有保存过快照的日期可以回溯。只有数据更新或导入新数据后才会追加记录。刷新、回溯和显示设置不会新增记录。`; }
...
function historyManagementKey(date, index) { return `${date}|${index}`; }
function parseHistoryManagementKey(key) { const separator = String(key).lastIndexOf('|'); if (separator < 0) return null; const date = String(key).slice(0, separator); const index = Number(String(key).slice(separator + 1)); return /^\d{4}-\d{2}-\d{2}$/.test(date) && Number.isInteger(index) && index >= 0 ? { date, index } : null; }
function historyManagementRecords() { const materialized = materializeHistory(state.history); return historyDates().flatMap(date => (materialized[date]?.versions || []).map((record, index) => ({ date, index, record }))); }
function renderDataManagement() {
  const list = document.getElementById('dataManagementList');
  const summary = document.getElementById('dataManagementSummary');
  const selectAll = document.getElementById('dataManagementSelectAll');
  const deleteButton = document.getElementById('dataManagementDeleteSelectedBtn');
  if (!list || !summary || !selectAll || !deleteButton) return;
  const records = historyManagementRecords();
  const available = new Set(records.map(item => historyManagementKey(item.date, item.index)));
  [...dataManagementSelection].forEach(key => { if (!available.has(key)) dataManagementSelection.delete(key); });
  const selectedCount = dataManagementSelection.size;
  summary.textContent = `${new Set(records.map(item => item.date)).size} 天 · ${records.length} 条记录 · 已选择 ${selectedCount} 条`;
  deleteButton.disabled = selectedCount === 0;
  selectAll.checked = records.length > 0 && selectedCount === records.length;
  selectAll.indeterminate = selectedCount > 0 && selectedCount < records.length;
  if (!records.length) { list.innerHTML = '<div class="data-empty">当前没有可管理的历史记录。</div>'; return; }
  const groups = new Map();
  records.forEach(item => { if (!groups.has(item.date)) groups.set(item.date, []); groups.get(item.date).push(item); });
  list.innerHTML = [...groups.entries()].map(([date, items]) => `<section class="data-date-group"><div class="data-date-head"><strong>${formatLongDate(date)} · ${date}</strong><span>${items.length} 条记录</span></div>${items.map(({ index, record }) => { const key = historyManagementKey(date, index); const checked = dataManagementSelection.has(key) ? ' checked' : ''; const eventCount = Array.isArray(record.events) ? record.events.length : 0; return `<div class="data-record-row"><label class="data-record-check"><input type="checkbox" class="data-record-check-input" data-history-key="${key}"${checked} /><span class="data-record-index">${index + 1}</span></label><div class="data-record-main"><strong>数据记录</strong><span>${formatSnapshotTime(record)}</span><small>${eventCount} 项事件 · 云端更新时间 ${formatSnapshotTime({ lastDataUpdateAt: record.lastDataUpdateAt })}</small></div><button type="button" class="data-delete-btn" data-history-delete="${key}" aria-label="删除 ${date} 第 ${index + 1} 条记录">×</button></div>`; }).join('')}</section>`).join('');
}
function openDataManagement() { dataManagementSelection.clear(); const backdrop = document.getElementById('dataManagementBackdrop'); if (!backdrop) return; backdrop.hidden = false; renderDataManagement(); }
function closeDataManagement() { const backdrop = document.getElementById('dataManagementBackdrop'); if (backdrop) backdrop.hidden = true; dataManagementSelection.clear(); }
function rebuildHistoryAfterDeletion(keys) {
  const removals = new Map();
  keys.map(parseHistoryManagementKey).filter(Boolean).forEach(({ date, index }) => { if (!removals.has(date)) removals.set(date, new Set()); removals.get(date).add(index); });
  if (!removals.size) return 0;
  const materialized = materializeHistory(state.history);
  const next = {};
  let removed = 0;
  historyDates().forEach(date => {
    const records = materialized[date]?.versions || [];
    const removeIndexes = removals.get(date) || new Set();
    const kept = records.filter((_, index) => { const shouldRemove = removeIndexes.has(index); if (shouldRemove) removed += 1; return !shouldRemove; });
    if (kept.length) next[date] = { versions: kept.map((record, index) => { const item = cloneData(record); delete item.date; delete item.versionIndex; delete item.baseDate; delete item.baseVersion; item.versionId = `${date}#${index + 1}`; return item; }) };
  });
  state.history = next;
  return removed;
}
function moveHistoryRecordsToTrash(keys) {
  const materialized = materializeHistory(state.history);
  const now = new Date().toISOString();
  const moved = keys.map(parseHistoryManagementKey).filter(Boolean).map(({ date, index }) => {
    const record = materialized[date]?.versions?.[index];
    if (!record) return null;
    const item = cloneData(record);
    delete item.date; delete item.versionIndex; delete item.baseDate; delete item.baseVersion;
    return { trashId: `${date}#${index}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, date, originalIndex: index, deletedAt: now, record: item };
  }).filter(Boolean);
  state.historyTrash = [...state.historyTrash, ...moved];
  return moved.length;
}
function deleteManagedHistoryRecords(keys) {
  const validKeys = keys.filter(key => parseHistoryManagementKey(key));
  if (!validKeys.length) return;
  const phrase = validKeys.length === 1 ? '这条记录' : `选中的 ${validKeys.length} 条记录`;
  if (!window.confirm(`确定删除${phrase}吗？同一天后面的记录会自动重新编号。删除会先应用到本机；如需同步所有设备，请随后导出云端历史文件并推送。`)) return;
  const moved = moveHistoryRecordsToTrash(validKeys);
  const removed = rebuildHistoryAfterDeletion(validKeys);
  if (!removed || !moved) return;
  dataManagementSelection.clear();
  saveHistorySnapshots();
  storage.removeItem('market-pulse-history-v1');
  if (state.historyDate) {
    const remaining = historyVersions(state.historyDate).length;
    if (!remaining) exitHistory();
    else state.historyVersion = Math.min(Number(state.historyVersion) || 0, remaining - 1);
  }
  render(true);
  renderDataManagement();
}
function exportCloudHistoryFile() {
  const payload = { schema: 'market-pulse-history', schemaVersion: 2, updatedAt: new Date().toISOString(), history: cloneData(state.history), trash: cloneData(state.historyTrash) };
  downloadJson('history.json', payload);
}
function renderTrash() {
  const list = document.getElementById('trashList');
  const summary = document.getElementById('trashSummary');
  const selectAll = document.getElementById('trashSelectAll');
  const restoreButton = document.getElementById('trashRestoreSelectedBtn');
  const deleteButton = document.getElementById('trashDeleteSelectedBtn');
  if (!list || !summary || !selectAll || !restoreButton || !deleteButton) return;
  const available = new Set(state.historyTrash.map(item => item.trashId));
  [...trashSelection].forEach(key => { if (!available.has(key)) trashSelection.delete(key); });
  const selectedCount = trashSelection.size;
  summary.textContent = `${state.historyTrash.length} 条记录 · 已选择 ${selectedCount} 条`;
  restoreButton.disabled = selectedCount === 0;
  deleteButton.disabled = selectedCount === 0;
  selectAll.checked = state.historyTrash.length > 0 && selectedCount === state.historyTrash.length;
  selectAll.indeterminate = selectedCount > 0 && selectedCount < state.historyTrash.length;
  if (!state.historyTrash.length) { list.innerHTML = '<div class="data-empty">回收站为空。</div>'; return; }
  const groups = new Map();
  state.historyTrash.forEach(item => { if (!groups.has(item.date)) groups.set(item.date, []); groups.get(item.date).push(item); });
  list.innerHTML = [...groups.entries()].map(([date, items]) => `<section class="data-date-group"><div class="data-date-head"><strong>${formatLongDate(date)} · ${date}</strong><span>${items.length} 条记录</span></div>${items.map(item => { const checked = trashSelection.has(item.trashId) ? ' checked' : ''; const eventCount = Array.isArray(item.record?.events) ? item.record.events.length : 0; return `<div class="data-record-row"><label class="data-record-check"><input type="checkbox" class="trash-record-check-input" data-trash-key="${escapeHtml(item.trashId)}"${checked} /><span class="data-record-index">${Number(item.originalIndex || 0) + 1}</span></label><div class="data-record-main"><strong>已删除记录</strong><span>删除时间：${formatSnapshotTime({ capturedAt: item.deletedAt })}</span><small>${eventCount} 项事件 · 原序号 ${Number(item.originalIndex || 0) + 1}</small></div><button type="button" class="data-delete-btn" data-trash-delete="${escapeHtml(item.trashId)}" aria-label="彻底删除记录">×</button></div>`; }).join('')}</section>`).join('');
}
function openTrash() { trashSelection.clear(); document.getElementById('dataManagementBackdrop').hidden = true; document.getElementById('trashBackdrop').hidden = false; renderTrash(); }
function closeTrash() { document.getElementById('trashBackdrop').hidden = true; trashSelection.clear(); }
function restoreTrashRecords(ids) {
  const selected = state.historyTrash.filter(item => ids.includes(item.trashId));
  if (!selected.length) return;
  if (!window.confirm(`确定还原选中的 ${selected.length} 条记录吗？还原后会按日期重新编号。`)) return;
  const materialized = materializeHistory(state.history);
  const grouped = new Map();
  selected.forEach(item => { if (!grouped.has(item.date)) grouped.set(item.date, []); grouped.get(item.date).push(item); });
  grouped.forEach((items, date) => {
    const records = materialized[date]?.versions ? materialized[date].versions.map(record => cloneData(record)) : [];
    items.sort((a, b) => Number(a.originalIndex || 0) - Number(b.originalIndex || 0)).forEach(item => { records.splice(Math.min(Number(item.originalIndex || 0), records.length), 0, cloneData(item.record)); });
    materialized[date] = { versions: records };
  });
  const next = {};
  Object.entries(materialized).forEach(([date, value]) => { if (value.versions.length) next[date] = { versions: value.versions.map((record, index) => { const item = cloneData(record); delete item.date; delete item.versionIndex; delete item.baseDate; delete item.baseVersion; item.versionId = `${date}#${index + 1}`; return item; }) }; });
  state.history = next;
  state.historyTrash = state.historyTrash.filter(item => !ids.includes(item.trashId));
  trashSelection.clear();
  saveHistorySnapshots();
  render(true);
  renderTrash();
}
function permanentlyDeleteTrashRecords(ids) {
  const selected = state.historyTrash.filter(item => ids.includes(item.trashId));
  if (!selected.length) return;
  if (!window.confirm(`确定彻底删除选中的 ${selected.length} 条记录吗？此操作无法恢复。`)) return;
  state.historyTrash = state.historyTrash.filter(item => !ids.includes(item.trashId));
  trashSelection.clear();
  saveHistorySnapshots();
  renderTrash();
}
function pruneHistory() { const all = []; const materialized = materializeHistory(state.history); Object.entries(materialized).forEach(([date, value]) => value.versions.forEach(record => all.push({ ...record, date }))); if (all.length <= 2) { window.alert('当前最多只有 2 条记录，无需清理。'); return; } if (!window.confirm(`将删除较早的 ${all.length - 2} 条记录，只保留最新 2 条记录用于前后对比。确定继续吗？`)) return; all.sort((a, b) => String(a.capturedAt || a.lastDataUpdateAt || '').localeCompare(String(b.capturedAt || b.lastDataUpdateAt || ''))); const keep = all.slice(-2); const next = {}; keep.forEach(record => { const { date, ...raw } = record; const item = cloneData(raw); delete item.versionIndex; delete item.baseDate; delete item.baseVersion; if (!next[date]) next[date] = { versions: [] }; item.versionId = `${date}#${next[date].versions.length + 1}`; next[date].versions.push(item); }); state.history = next; saveHistorySnapshots(); storage.removeItem('market-pulse-history-v1'); if (state.historyDate && !state.history[state.historyDate]) exitHistory(); else render(true); window.alert(`已清理 ${all.length - keep.length} 条旧记录，保留最新 2 条记录。`); }
function archivePayload() { return { schema: 'market-pulse-investment-calendar-archive', schemaVersion: 2, exportedAt: new Date().toISOString(), events: state.events.filter(event => !event.seed).map(event => cloneData(event)), updates: cloneData(readStorageJson('market-pulse-updates-v1', {})), history: cloneData(state.history), historyTrash: cloneData(state.historyTrash), preferences: { filters: cloneData(state.filters), subfilters: cloneData(state.subfilters), timezone: state.timezone, language: state.language } }; }
...
document.getElementById('dataManagementBtn').addEventListener('click', () => { toggleStorageMenu(false); openDataManagement(); });
...
document.getElementById('closeTrashBtnBottom').addEventListener('click', closeTrash);
document.getElementById('trashBackdrop').addEventListener('click', event => { if (event.target.id === 'trashBackdrop') closeTrash(); });
document.getElementById('trashSelectAll').addEventListener('change', event => { state.historyTrash.forEach(item => { if (event.target.checked) trashSelection.add(item.trashId); else trashSelection.delete(item.trashId); }); renderTrash(); });
document.getElementById('trashRestoreSelectedBtn').addEventListener('click', () => restoreTrashRecords([...trashSelection]));
document.getElementById('trashDeleteSelectedBtn').addEventListener('click', () => permanentlyDeleteTrashRecords([...trashSelection]));
document.getElementById('trashList').addEventListener('change', event => { const input = event.target.closest('.trash-record-check-input'); if (!input) return; if (input.checked) trashSelection.add(input.dataset.trashKey); else trashSelection.delete(input.dataset.trashKey); renderTrash(); });
document.getElementById('trashList').addEventListener('click', event => { const button = event.target.closest('[data-trash-delete]'); if (!button) return; permanentlyDeleteTrashRecords([button.dataset.trashDelete]); });
...
document.getElementById('closeDataManagementBtnBottom').addEventListener('click', closeDataManagement);
document.getElementById('dataManagementBackdrop').addEventListener('click', event => { if (event.target.id === 'dataManagementBackdrop') closeDataManagement(); });
document.getElementById('dataManagementSelectAll').addEventListener('change', event => { historyManagementRecords().forEach(item => { const key = historyManagementKey(item.date, item.index); if (event.target.checked) dataManagementSelection.add(key); else dataManagementSelection.delete(key); }); renderDataManagement(); });
document.getElementById('dataManagementDeleteSelectedBtn').addEventListener('click', () => deleteManagedHistoryRecords([...dataManagementSelection]));
document.getElementById('dataManagementExportBtn').addEventListener('click', exportCloudHistoryFile);
document.getElementById('dataManagementList').addEventListener('change', event => { const input = event.target.closest('.data-record-check-input'); if (!input) return; if (input.checked) dataManagementSelection.add(input.dataset.historyKey); else dataManagementSelection.delete(input.dataset.historyKey); renderDataManagement(); });
document.getElementById('dataManagementList').addEventListener('click', event => { const button = event.target.closest('[data-history-delete]'); if (!button) return; deleteManagedHistoryRecords([button.dataset.historyDelete]); });
...
document.addEventListener('keydown', event => { if (event.key === 'Escape') { closeHistoryPicker(); closeTrash(); } if (event.target.matches('input, textarea, select')) return; if (event.key === 'PageDown') changeMonth(1); if (event.key === 'PageUp') changeMonth(-1); });
